...
class SiteConfig:
  ...
  # Identity
  base_name: str = "Antenna Removal"
  brand_name: str = "Antenna Removal Experts"
  ...
  h1_short: str = "Antenna Removal Services"

  cost_title: str = "Antenna Removal Cost"
  howto_title: str = "How Antenna Removal Works"

  about_blurb: str = (
    "We provide professional antenna removal services{loc} for homeowners with old rooftop TV antennas and mounts that need to come down. "
    "Our service removes the antenna safely, addresses mounting hardware, and helps ensure the roof is left weather-tight afterward. "
    "Request a quick quote and schedule service with a trusted local antenna removal professional."
  )

  # MAIN PAGE (high-intent, Ahrefs/serp-aligned)
  main_h2: tuple[str, ...] = (
    "Who Removes Rooftop TV Antennas?",
    "Can You Remove an Antenna From a Roof Without Damaging It?",
    "What Happens to the Mounting Bracket and Bolt Holes After Antenna Removal?",
    "How Is TV Antenna Removal Different From Satellite Dish Removal?",
    "When Should an Old TV Antenna or Antenna Tower Be Removed?",
    "How Do You Know If a Roof Antenna Is Loose or Unsafe?",
  )

  main_p: tuple[str, ...] = (
    "Rooftop TV antennas are typically removed by a professional antenna removal service or a qualified handyman who can work safely on the roof. This is especially important when the antenna is tall, corroded, or mounted near roof edges or electrical lines.",
    "Yes, an antenna can usually be removed without damaging the roof when it’s taken down carefully and the mounting points are handled correctly. The key is removing the hardware cleanly and avoiding tearing shingles, bending flashing, or leaving exposed fasteners behind.",
    "After antenna removal, the mounting bracket is taken off and the attachment points are sealed so water can’t get in. Any bolt holes or penetrations should be properly patched and sealed to reduce leak risk and prevent future roof damage.",
    "TV antenna removal usually involves a mast, bracket, and rooftop mounting hardware, while satellite dish removal often includes a dish arm, base plate, and coax cable routing. Both can leave roof penetrations, but the mounting styles and cable paths are often different, which changes how the roof is sealed afterward.",
    "An old TV antenna or antenna tower should be removed when it’s rusted, leaning, loose in high winds, or no longer in use. Removing unused hardware can reduce storm risk and helps avoid roof leaks caused by aging mounts and seals.",
    "A roof antenna may be loose or unsafe if it wobbles in the wind, shows visible rust at the base, or has shifting brackets or missing fasteners. If you notice movement, noise, or lifted shingles around the mount, it’s a good sign the antenna needs attention."
  )

  # LOCAL COST
  location_cost_h2: str = "How Much Does Antenna Removal Cost{loc}?"
  location_cost_p: str = (
    "Antenna removal costs{loc} typically range between {cost_lo} and {cost_hi}, depending on roof height, antenna size, and how the mount is attached. "
    "Pricing can also vary based on roof access, safety requirements, and whether roof sealing is needed after removal."
  )

  # Meta descriptions (keep them unique per page, ~150-160 chars)
  home_description: str = (
    "Professional TV antenna removal for rooftops. We remove old antennas safely and help leave the roof sealed and weather-tight. Request a quote."
  )

  city_description: str = (
    "Antenna removal services{loc} for rooftop TV antennas. Safe removal, mount handling, and roof sealing support available. Get a quote."
  )

  cost_description: str = (
    "Antenna removal cost guide: typical price ranges and factors like roof height, antenna size, access, and sealing after removal."
  )

  cost_city_description: str = (
    "Antenna removal cost{loc}: typical local pricing ranges and factors like roof access, antenna mounts, and roof sealing needs."
  )

  howto_description: str = (
    "How antenna removal works: an overview of safe rooftop removal, mount hardware handling, and sealing around roof penetrations."
  )

  contact_description: str = (
    "Request a quote for antenna removal. Share your location and roof details to get pricing and scheduling for rooftop TV antenna removal."
  )

  state_description: str = (
    "Antenna removal services{loc}. Browse cities we serve, view typical pricing ranges, and request a quote."
  )
...
<div id="networx_form_container" style="margin:0px;padding:0px;">
    <div id = "nx_form" style = "width: 242px; height: 375px;">
        <script type="text/javascript" src = "https://api.networx.com/iframe.php?aff_id=73601bc3bd5a961a61a973e92e29f169&aff_to_form_id=8029"></script>
    </div>
</div>
""".strip()
